    ...
    >
      ...
      <nav className="flex-1 py-3 px-2 overflow-y-auto">
        ...
        <div className="space-y-0.5">
          {MAIN_NAV.filter((i) => i.href === "/ideas").map((item) => (
        ...
        <div className="mt-5">
          ...
          <div className="space-y-0.5">
            {MAIN_NAV.filter((i) => ["/frameworks", "/prior-art"].includes(i.href)).map((item) => (
              <NavLink key={item.href} item={item} active={pathname.startsWith(item.href)} collapsed={collapsed} />
            ))}
          </div>
        </div>
        ...
        <div className="mt-5">
          ...
          {collapsed && <div className="mx-2 my-2 h-px bg-border" />}
          <div className="space-y-0.5">
            {MAIN_NAV.filter((i) => i.href === "/sprints").map((item) => (
              <NavLink key={item.href} item={item} active={pathname.startsWith(item.href)} collapsed={collapsed} />
            ))}
          </div>
        </div>
      ...
      <div className="px-3 py-3 mt-2 border-t border-border">
        {!collapsed ? (
          <p className="text-xs text-neutral-light">IP Ramp Open Source</p>
        ) : (
