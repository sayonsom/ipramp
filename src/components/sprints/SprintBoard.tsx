        ...
        <div className="flex-1 min-w-0 space-y-3">
            ...
            <div className="flex gap-1 overflow-x-auto">
              <button
                ...
                className={`px-2.5 py-1 rounded-md text-xs font-medium transition-colors ${
                  activePhase === "all" ? "bg-ink text-white" : "text-text-muted hover:text-ink"
                }`}
              >
                All ({ideas.length})
              </button>
              {phaseCounts.map((p) => (
                ...
                >
                  {p.label}
                  {p.count > 0 && (
                    <span className="rounded-full px-1 text-[10px]" style={{ background: p.color + "25", color: p.color }}>
                      {p.count}
                    </span>
                  )}
                </button>
          ...
          {sorted.length === 0 ? (
            <div className="text-center py-16 text-text-muted">
              <p className="text-sm">
                {ideas.length === 0 ? "No ideas yet. Start brainstorming above!" : "No ideas in this phase."}
              </p>
            </div>
          ) : (
              ...
              {sorted.map((idea) => (
                <SprintIdeaCard
                  ...
                  onAdvance={() => {
                    ...
                    if (idx < SPRINT_PHASES.length - 1) updateIdeaPhase(idea.id, SPRINT_PHASES[idx + 1].key);
                  }}
                  onRetreat={() => {
                    const idx = SPRINT_PHASES.findIndex((p) => p.key === idea.phase);
                    if (idx > 0) updateIdeaPhase(idea.id, SPRINT_PHASES[idx - 1].key);
                  }}
          ...
          <div className="bg-white rounded-lg border border-border">
            ...
            >
              <div className="flex items-center gap-2">
                <span className="text-xs font-medium text-text-muted uppercase tracking-wider">Unlinked Ideas</span>
                {candidates.length > 0 && (
            ...
            {showCandidates && (
              <div className="border-t border-border p-3 space-y-2">
                {candidates.length === 0 ? (
                  <p className="text-xs text-text-muted text-center py-4">No unlinked ideas.</p>
                ) : (
                  <>
                    <Input
              ...
              <div className="flex gap-px h-5 rounded overflow-hidden mb-3">
                {phaseCounts.filter((p) => p.count > 0).map((p) => (
                  <div
                    ...
                    style={{ flex: p.count, backgroundColor: p.color + "30", color: p.color }}
          ...
          >
            ...
            <div className="flex flex-wrap gap-1">
              {currentMode.rules.map((r, i) => (
                <span key={i} className="text-[10px] text-neutral-dark bg-white/80 rounded px-1.5 py-0.5">{r}</span>
              ))}
            </div>
          </div>
...
function SprintIdeaCard({ idea, creatorName, onAdvance, onRetreat, onRemove }: {
  idea: Idea; creatorName: string; onAdvance: () => void; onRetreat: () => void; onRemove: () => void;
}) {
  ...
  return (
    ...
    >
      <div className="flex items-start gap-3">
        <div className="w-1 self-stretch rounded-full shrink-0 mt-0.5" style={{ backgroundColor: phase.color }} />
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-1.5 mb-0.5">
            <Link href={`/ideas/${idea.id}`} className="text-sm font-medium text-ink hover:text-blue-ribbon transition-colors truncate">
              {idea.title || "Untitled"}
            </Link>
            {isDraft && (
              <Link href={`/ideas/${idea.id}`} className="text-[10px] text-blue-ribbon hover:underline shrink-0">
                Enrich &rarr;
              </Link>
            )}
          </div>
          <div className="flex items-center gap-2 text-[11px] text-text-muted">
            ...
            {idea.frameworkUsed !== "none" && (
              <><span className="text-border">&middot;</span><span>{idea.frameworkUsed.toUpperCase()}</span></>
            )}
            {idea.problemStatement && !isDraft && (
              <><span className="text-border">&middot;</span><span className="truncate">{idea.problemStatement.slice(0, 60)}</span></>
            )}
          </div>
        </div>
        <div className="flex items-center gap-2 shrink-0">
          {progress.completed > 0 && (
            <div className="flex gap-px" title={`${progress.completed}/${progress.total} stages`}>
              {Array.from({ length: progress.total }).map((_, i) => (
                <div key={i} className={`w-1 h-2.5 rounded-sm ${i < progress.completed ? "bg-blue-ribbon" : "bg-gray-200"}`} />
              ))}
            </div>
          )}
          {total > 0 && (
            <span className="text-[11px] font-semibold font-mono px-1.5 py-0.5 rounded" style={{ backgroundColor: verdict.color + "15", color: verdict.color }}>
              {total}/9
            </span>
          )}
          {idea.aliceScore && (
            <span className={`text-[10px] font-medium px-1.5 py-0.5 rounded ${
              idea.aliceScore.abstractIdeaRisk === "low" ? "bg-green-50 text-green-700" :
              idea.aliceScore.abstractIdeaRisk === "medium" ? "bg-amber-50 text-amber-700" : "bg-red-50 text-red-700"
            }`}>
          ...
          <div className={`flex gap-0.5 transition-opacity ${hovered ? "opacity-100" : "opacity-0"}`}>
            {phaseIdx > 0 && (
              <button onClick={onRetreat} className="w-5 h-5 rounded flex items-center justify-center text-text-muted hover:bg-gray-100 hover:text-ink transition-colors" title={`Move to ${SPRINT_PHASES[phaseIdx - 1].label}`}>
            ...
            {phaseIdx < SPRINT_PHASES.length - 1 && (
              <button onClick={onAdvance} className="w-5 h-5 rounded flex items-center justify-center text-text-muted hover:bg-gray-100 hover:text-ink transition-colors" title={`Move to ${SPRINT_PHASES[phaseIdx + 1].label}`}>
            ...
            <button onClick={onRemove} className="w-5 h-5 rounded flex items-center justify-center text-text-muted hover:bg-red-50 hover:text-red-500 transition-colors" title="Remove from sprint">
...
function CandidateIdeaCard({ idea, creatorName, onAdd }: { idea: Idea; creatorName: string; onAdd: () => void }) {
  const total = getTotalScore(idea.score);
  const verdict = getScoreVerdict(total);
    ...
    <div className="flex items-center gap-2.5 px-2.5 py-2 rounded-md hover:bg-gray-50 transition-colors group">
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-1.5">
          <Link href={`/ideas/${idea.id}`} className="text-xs font-medium text-ink hover:text-blue-ribbon truncate">{idea.title}</Link>
          {total > 0 && (
            <span className="text-[10px] font-mono px-1 py-0.5 rounded shrink-0" style={{ backgroundColor: verdict.color + "15", color: verdict.color }}>
              {total}/9
            </span>
          )}
        </div>
      ...
      <button onClick={onAdd} className="text-[11px] font-medium text-blue-ribbon hover:text-blue-ribbon/80 opacity-0 group-hover:opacity-100 transition-opacity shrink-0">
        + Add
      </button>
    </div>
  );
